...
def regression_cubic_poly(num_train=20, num_test=100, train_domain=(-3.5, 3.5),
    ...
    train_x = rand.uniform(
        low=train_domain[0], high=train_domain[1], size=(num_train, 1)
    )
    test_x = np.linspace(
        start=test_domain[0], stop=test_domain[1], num=num_test
    ).reshape((num_test, 1))
...
def computeELBO(net, predictions, targets, device, args):
    ...
    # Be aware of the reduction='mean' in the computation of the mse loss
    # below.
...
def plot_predictions(device, test_loader, trainings_loader, net, args):
    ...
    assert(
        training_data.inputs.shape[1] == 1
        and training_data.outputs.shape[1] == 1
    )
    ...
    with torch.no_grad():
        ...
        test_predictions = net.forward(
            test_data.inputs, args.bbb
        ).detach().cpu().numpy()
    ...
    plt.errorbar(
        test_inputs, test_predictions, np.sqrt(error_bar), color='r',
        label='Predictions'
    )
    ...
    plt.scatter(
        [3.5]*100+[-3.5]*100, borders, color='k', s=0.1,
