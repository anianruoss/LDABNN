...
maximum likelihood and a bayesian treatment of learning. For the latter, we
need to approximate the intractable weight posterior to do inference.
...
collected and in what state your algorithm was when we started training (one
could even imagine that the algorithm was already trained on another task).
    ...
    for e in range(args.epochs):
        for inputs, targets in train_loader:
            ...
            if args.bbb:
                ...
                nll, kl = utils.computeELBO(
                    net, predictions, targets, device, args
                )
                loss = nll + kl
            else:
                kl = 0
...
def run():
    ...
    train_loader = DataLoader(
        utils.RegressionDataset(train_x, train_y), batch_size=args.batch_size,
        shuffle=True
    )
    test_loader = DataLoader(
        utils.RegressionDataset(test_x, test_y), batch_size=args.batch_size,
        shuffle=False
    )
